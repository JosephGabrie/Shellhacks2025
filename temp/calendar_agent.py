...
list_reminders_tool = FunctionTool(func=list_reminders)
add_reminder_tool   = FunctionTool(func=add_reminder)
freebusy_tool       = FunctionTool(func=freebusy)
...
if __name__ == "__main__":
    ...
    if not os.getenv("GOOGLE_GENAI_API_KEY"):
        print("[error] Missing GOOGLE_GENAI_API_KEY env var.")
    ...
    agent = Agent(
        ...
        tools=[list_reminders_tool, add_reminder_tool, freebusy_tool],
    )

    async def run_adk():
        session_service = InMemorySessionService()
        await session_service.create_session(app_name=args.app_name, user_id=args.user_id, session_id=args.session_id)
        runner = Runner(agent=agent, app_name=args.app_name, session_service=session_service)
